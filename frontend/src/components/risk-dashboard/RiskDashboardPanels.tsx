...
type DepletionPanelProps = {
    depletionAnalysis: DepletionAnalysis | null;
};

export function DepletionPanel({ depletionAnalysis }: DepletionPanelProps) {
    return (
...
export function SensitivityPanel({
    ...
    return (
        <div className="animate-in fade-in duration-300">
            ...
            <button onClick={onRunSensitivity} disabled={runningAnalysis} className="inline-flex items-center justify-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-semibold text-sm transition-all active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed mb-4">
...
export function SorrPanel({ input, onInputChange }: SorrPanelProps) {
    return (
        <div className="animate-in fade-in duration-300">
            <p className="text-slate-500 dark:text-slate-400 text-sm mb-4 flex items-center gap-2">
...
type MedicalShockPanelProps = {
    input: SimulationInput;
    onToggleMedicalShock: (enabled: boolean) => void;
    onAddMedicalShock: () => void;
    onRemoveMedicalShock: (index: number) => void;
    onUpdateMedicalShock: (index: number, field: "age" | "amount" | "description", value: string | number) => void;
};

export function MedicalShockPanel({
    input,
    onToggleMedicalShock,
    onAddMedicalShock,
    onRemoveMedicalShock,
    onUpdateMedicalShock,
}: MedicalShockPanelProps) {
    return (
        <div className="animate-in fade-in duration-300">
            ...
            <div className="mb-6 flex items-center">
                <label className="flex items-center gap-3 text-sm cursor-pointer font-semibold text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-slate-100 group">
                    <input
                        ...
                        onChange={(event) => onToggleMedicalShock(event.target.checked)}
                    />
                    의료비 쇼크 시나리오 활성화
                </label>
            </div>
            ...
            {input.medical_shocks?.enabled && (
                <div className="flex flex-col gap-3">
                    {input.medical_shocks.occurrences.map((shock, index) => (
                        <div key={index} className="flex flex-wrap items-center gap-2 bg-slate-50 dark:bg-zinc-800/50 p-2 lg:p-3 rounded-xl border border-slate-100 dark:border-zinc-800">
                            <input
                                ...
                                value={shock.age}
                                onChange={(event) => onUpdateMedicalShock(index, "age", Number(event.target.value))}
                                placeholder="연령"
                            />
                            ...
                            <input
                                ...
                                value={shock.amount}
                                onChange={(event) => onUpdateMedicalShock(index, "amount", Number(event.target.value))}
                                placeholder="금액"
                            />
                            ...
                            <input
                                ...
                                onChange={(event) => onUpdateMedicalShock(index, "description", event.target.value)}
                                placeholder="설명 (예: 수술비)"
                            />
                            <button
                                onClick={() => onRemoveMedicalShock(index)}
                    ...
                    <button onClick={onAddMedicalShock} className="self-start mt-2 inline-flex items-center justify-center gap-2 px-3 py-1.5 bg-slate-50 dark:bg-zinc-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-zinc-700 hover:bg-slate-100 dark:hover:bg-zinc-700 hover:border-slate-300 dark:hover:border-zinc-600 rounded-lg text-sm font-semibold transition-all active:scale-[0.98] cursor-pointer">
                        + 항목 추가
                    </button>
                </div>
            )}
